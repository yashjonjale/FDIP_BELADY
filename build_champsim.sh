...
if [ "$#" -ne 9 ]; then
    ...
    echo "Usage: ./build_champsim.sh [branch_pred] [btb] [l1d_pref] [l2c_pref] [llc_pref] [llc_repl] [l1i_repl] [num_core]"
    exit 1
fi
...
BTB=$2              # btb/*.btb
L1I_PREFETCHER=$3   # prefetcher/*.l1i_pref
L1D_PREFETCHER=$4   # prefetcher/*.l1d_pref
L2C_PREFETCHER=$5   # prefetcher/*.l2c_pref
LLC_PREFETCHER=$6   # prefetcher/*.llc_pref
LLC_REPLACEMENT=$7  # replacement/*.llc_repl
L1I_REPLACEMENT=$8  # replacement/*.l1i_repl
...
echo "Cores: ${NUM_CORE}"
BINARY_NAME="${BRANCH}-${BTB}-${L1I_PREFETCHER}-${L1D_PREFETCHER}-${L2C_PREFETCHER}-${LLC_PREFETCHER}-${LLC_REPLACEMENT}-${L1I_REPLACEMENT}-${NUM_CORE}core"
