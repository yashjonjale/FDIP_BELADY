...
void print_roi_stats(uint32_t cpu, CACHE *cache)
{
    ...
    cout << cache->NAME;
    cout << " PREFETCH  REQUESTED: " << setw(10) << cache->pf_requested << "  ISSUED: " << setw(10) << cache->pf_issued << endl;
    cout << "XXX " << cache->NAME << "-USEFUL " << cache->pf_useful << std::endl << "XXX " << cache->NAME << "-USELESS " << cache->pf_useless << endl;
...
void print_branch_stats()
{
	...
	cout << "XXX total_mispredicts " << ooo_cpu[i].branch_mispredictions << endl;
	cout << "XXX total_target_mispredicts1 " << ooo_cpu[i].branch_target_mispredictions1 << endl;
	cout << "XXX total_target_mispredicts2 " << ooo_cpu[i].branch_target_mispredictions2 << endl;
	cout << "XXX total_direction_mispredicts " << ooo_cpu[i].branch_direction_mispredictions << endl;
	cout << "XXX mispredict_on_btb_hit " << ooo_cpu[i].mispredict_on_btb_hit << endl;

	cout << "XXX NOT_BRANCH: " << ooo_cpu[i].mispredicted_branch_types[0] << endl;
	cout << "XXX BRANCH_DIRECT_JUMP: " << ooo_cpu[i].mispredicted_branch_types[1] << endl;
	cout << "XXX BRANCH_INDIRECT: " << ooo_cpu[i].mispredicted_branch_types[2] << endl;
	cout << "XXX BRANCH_CONDITIONAL: " << ooo_cpu[i].mispredicted_branch_types[3] << endl;
	cout << "XXX BRANCH_DIRECT_CALL: " << ooo_cpu[i].mispredicted_branch_types[4] << endl;
	cout << "XXX BRANCH_INDIRECT_CALL: " << ooo_cpu[i].mispredicted_branch_types[5] << endl;
	cout << "XXX BRANCH_RETURN: " << ooo_cpu[i].mispredicted_branch_types[6] << endl;
	cout << "XXX BRANCH_OTHER: " << ooo_cpu[i].mispredicted_branch_types[7] << endl;
	...
	cout << "XXX BTB_reads: " << ooo_cpu[i].BTB_reads << endl;
	cout << "XXX BTB_writes: " << ooo_cpu[i].BTB_writes << endl;
	cout << "XXX PageBTB_reads: " << ooo_cpu[i].PageBTB_reads << endl;
	cout << "XXX PageBTB_writes: " << ooo_cpu[i].PageBTB_writes << endl;
	cout << "XXX PageBTB_readsBeforeWrite: " << ooo_cpu[i].PageBTB_readsBeforeWrite << endl;
	cout << "XXX RegionBTB_reads: " << ooo_cpu[i].RegionBTB_reads << endl;
	cout << "XXX RegionBTB_writes: " << ooo_cpu[i].RegionBTB_writes << endl;
	cout << "XXX RegionBTB_readsBeforeWrite: " << ooo_cpu[i].RegionBTB_readsBeforeWrite << endl;
	/*std::map<uint64_t, std::set<uint64_t>>::iterator it;
	uint64_t numBranches[17] = {0};
	uint64_t total_CB, total_branches = 0;
	total_CB = ooo_cpu[i].branchesInCB.size();
	for (it = ooo_cpu[i].branchesInCB.begin(); it != ooo_cpu[i].branchesInCB.end(); it++) {
		assert(it->second.size() < 17);
		numBranches[it->second.size()]++;
		total_branches += it->second.size();
	}*/
	for (int j = 0; j < 17; j++) {
		//cout << "XXX CBbranch" << j << " " << numBranches[j] << endl;
	}
	//cout << "XXX TotalBranches " << total_branches << endl;
	//cout << "XXX TotalCB " << total_CB << endl;

    for(int j = 0; j < 132; j++) {
//         cout << "XXX disp-" << j << " " << ooo_cpu[i].offset_size[j] << endl;
	}

	cout << "XXX BTB_4D_Hits " << ooo_cpu[i].BTB_4D_Hits << endl;
	cout << "XXX BTB_6D_Hits " << ooo_cpu[i].BTB_6D_Hits << endl;
	cout << "XXX BTB_8D_Hits " << ooo_cpu[i].BTB_8D_Hits << endl;
	cout << "XXX BTB_12D_Hits " << ooo_cpu[i].BTB_12D_Hits << endl;
	cout << "XXX BTB_18D_Hits " << ooo_cpu[i].BTB_18D_Hits << endl;
	cout << "XXX BTB_25D_Hits " << ooo_cpu[i].BTB_25D_Hits << endl;
	cout << "XXX BTB_46D_Hits " << ooo_cpu[i].BTB_46D_Hits << endl;
	cout << "XXX BTB_Ret_Hits " << ooo_cpu[i].BTB_Ret_Hits << endl;

	cout << "XXX BTB_4D_Misses " << ooo_cpu[i].BTB_4D_Misses << endl;
	cout << "XXX BTB_6D_Misses " << ooo_cpu[i].BTB_6D_Misses << endl;
	cout << "XXX BTB_8D_Misses " << ooo_cpu[i].BTB_8D_Misses << endl;
	cout << "XXX BTB_12D_Misses " << ooo_cpu[i].BTB_12D_Misses << endl;
	cout << "XXX BTB_18D_Misses " << ooo_cpu[i].BTB_18D_Misses << endl;
	cout << "XXX BTB_25D_Misses " << ooo_cpu[i].BTB_25D_Misses << endl;
	cout << "XXX BTB_46D_Misses " << ooo_cpu[i].BTB_46D_Misses << endl;
...
void finish_warmup()
{
    ...
    for (uint32_t i=0; i<NUM_CPUS; i++) {
        ...
        ooo_cpu[i].branch_target_mispredictions1 = 0;
        ooo_cpu[i].branch_target_mispredictions2 = 0;
		ooo_cpu[i].branch_direction_mispredictions = 0;
		ooo_cpu[i].mispredict_on_btb_hit = 0;
	ooo_cpu[i].total_rob_occupancy_at_branch_mispredict = 0;
	ooo_cpu[i].branchesInCB.clear();
	ooo_cpu[i].unique_branchPC.clear();


	for(uint32_t j=0; j<132; j++)
	  {
	    ooo_cpu[i].offset_size[j] = 0;
	  }

	for(uint32_t j=0; j<8; j++)
	  {
	    ooo_cpu[i].total_branch_types[j] = 0;
	    ooo_cpu[i].mispredicted_branch_types[j] = 0;
	  }
	
	ooo_cpu[i].BTB_reads = ooo_cpu[i].BTB_writes = ooo_cpu[i].PageBTB_reads = ooo_cpu[i].PageBTB_writes = ooo_cpu[i].PageBTB_readsBeforeWrite = ooo_cpu[i].RegionBTB_reads = ooo_cpu[i].RegionBTB_writes = ooo_cpu[i].RegionBTB_readsBeforeWrite = 0;
        ...
        ooo_cpu[i].BTB_4D_Hits = ooo_cpu[i].BTB_6D_Hits = ooo_cpu[i].BTB_8D_Hits = ooo_cpu[i].BTB_12D_Hits = ooo_cpu[i].BTB_18D_Hits = ooo_cpu[i].BTB_25D_Hits = ooo_cpu[i].BTB_46D_Hits = ooo_cpu[i].BTB_Ret_Hits = 0;
        ooo_cpu[i].BTB_4D_Misses = ooo_cpu[i].BTB_6D_Misses = ooo_cpu[i].BTB_8D_Misses = ooo_cpu[i].BTB_12D_Misses = ooo_cpu[i].BTB_18D_Misses = ooo_cpu[i].BTB_25D_Misses = ooo_cpu[i].BTB_46D_Misses = ooo_cpu[i].BTB_Ret_Misses = 0;
    }
    cout << endl;
...
int main(int argc, char** argv)
{
	...
	ooo_cpu[i].ITLB.MAX_READ = 2;
        ...
        ooo_cpu[i].L1I.l1i_initialize_replacement();///Yash added initialization for replacement policy
	      ...
	      if(ooo_cpu[i].DECODE_BUFFER.occupancy > 0)
		{
	      ...
	      /*if ((ooo_cpu[i].IFETCH_BUFFER.occupancy < ooo_cpu[i].IFETCH_BUFFER.SIZE) && (ooo_cpu[i].fetch_stall == 0))
	      {
              ...
              if ((ooo_cpu[i].IFETCH_BUFFER.occupancy < ooo_cpu[i].IFETCH_BUFFER.SIZE))
              {
                  ...
                  {
                          ooo_cpu[i].spec_inst_prefetch();///this happens when there is a fetch stall
                  }
              }
    ...
    for (uint32_t i=0; i<NUM_CPUS; i++) {
        cout << endl << "CPU " << i << std::endl << "XXX cumulative-IPC " << ((float) ooo_cpu[i].finish_sim_instr / ooo_cpu[i].finish_sim_cycle) << std::endl;
        cout << "XXX instructions " << ooo_cpu[i].finish_sim_instr << std::endl << "XXX cycles " << ooo_cpu[i].finish_sim_cycle << endl;
#ifndef CRC2_COMPILE
    ...
    for (uint32_t i=0; i<NUM_CPUS; i++) {
        ...
        ooo_cpu[i].L1D.l1d_prefetcher_final_stats();
        ooo_cpu[i].L2C.l2c_prefetcher_final_stats();
        ooo_cpu[i].L1I.l1i_replacement_final_stats();///Yash added l1i replacement final stats
    }
