...
void O3_CPU::read_from_trace()
{
    ...
    if (skip_next_cycle)
    {
        ...
        //		cout << "Skipping cycle " << current_core_cycle[cpu] << endl;
        return;
    }
    ...
    while (continue_reading)
    {
        ...
        else
        {
            ...
            { // successfully read the trace
                ...
                // add this instruction to the IFETCH_BUFFER   ///Yash - FTQ fill happens here
                if (IFETCH_BUFFER.occupancy < IFETCH_BUFFER.SIZE)
                {
                    uint32_t ifetch_buffer_index = add_to_ifetch_buffer(&arch_instr);
                    num_reads++;
                    ...
                    // Call code prefetch
                    nextInstPC = next_instr.ip;
                    IBUF_occupancy = IFETCH_BUFFER.occupancy;
                    l1i_prefetcher_instruction_operate(IFETCH_BUFFER.entry[ifetch_buffer_index].ip);///Yash - Call to FDIP code prefetcher
                    assert(IFETCH_BUFFER.entry[ifetch_buffer_index].ip == arch_instr.ip);
                    ...
                    if (IFETCH_BUFFER.entry[ifetch_buffer_index].is_branch)
                    {
                        ...
                        num_branch++;
                        unique_branchPC.insert(IFETCH_BUFFER.entry[ifetch_buffer_index].ip);

                        // handle branch prediction & branch predictor update
                        bool btb_hit = false;
                        bool is_btb_hit = true;
                        BTB_outcome btb_result = btb_prediction(arch_instr.ip, arch_instr.branch_type);
                        /*BTB_outcome btb_result;
                        if (arch_instr.branch_taken) {
                btb_result = btb_prediction(arch_instr.ip, arch_instr.branch_type);
            } else {
                btb_result = {0, BRANCH_CONDITIONAL, 0};
            }*/
                        uint64_t predicted_branch_target = btb_result.target;
                        uint64_t predicted_branch_target_org = predicted_branch_target;
                        uint8_t branch_type = btb_result.branch_type;
                        uint8_t branch_prediction = predict_branch(IFETCH_BUFFER.entry[ifetch_buffer_index].ip);
                        if (branch_type != BRANCH_CONDITIONAL)
                        {
                            branch_prediction = 1;
                        }
                        if (btb_result.sequential_BTB_access == 2)
                        {
                            is_btb_hit = false;
                        }

                        if (btb_result.sequential_BTB_access == 2 && arch_instr.branch_taken)
                        {
                            /*This was a BTB miss*/
                            btb_hit = false;
                            btb_result.sequential_BTB_access = 0;

                            if (arch_instr.branch_type != BRANCH_RETURN)
                            {
                                uint64_t diff_bits = (arch_instr.branch_target >> 2) ^ (arch_instr.ip >> 2);
                                int num_bits = 0;
                                while (diff_bits != 0)
                                {
                        ...
                        else if (arch_instr.branch_taken)
                        {
                            /*This was a BTB hit*/
                            btb_hit = true;
                        ...
                        if (branch_prediction && btb_result.sequential_BTB_access == 1)
                        {
                            //                        	cout << "Need to skip next cycle. Current " << current_core_cycle[cpu] << endl;
                        ...
                        l1i_prefetcher_branch_operate(IFETCH_BUFFER.entry[ifetch_buffer_index].ip,
                                                      ...
                                                      arch_instr.branch_target /*predicted_branch_target*/);///Yash - Very Crucial - For Precise FDIP running
                        ///Yash you may pass bpu predictions to prefetcher in this way, perhaps for fdip precise
                        // if(IFETCH_BUFFER.entry[ifetch_buffer_index].branch_taken != branch_prediction)
                        if (arch_instr.branch_target != predicted_branch_target)
                        {
                            branch_mispredictions++;
                            if (IFETCH_BUFFER.entry[ifetch_buffer_index].branch_type != BRANCH_CONDITIONAL)
                            {
                                branch_target_mispredictions1++;
                                if (btb_hit)
                                {
                                    mispredict_on_btb_hit++;
                                    mispredicted_branch_types[IFETCH_BUFFER.entry[ifetch_buffer_index].branch_type]++;
                                    assert(arch_instr.branch_taken);
                                }
                            }
                            else
                            {
                                ...
                                {
                                    if (arch_instr.branch_target != predicted_branch_target_org)
                                    {
                                        branch_target_mispredictions2++;
                                    }
                                    else
                                    {
                            ...
                            if (warmup_complete[cpu])
                            {
                                ...
                                instrs_to_read_this_cycle = 0;
                                IFETCH_BUFFER.entry[ifetch_buffer_index].branch_mispredicted = 1;
                                if (is_btb_hit == false && IFETCH_BUFFER.entry[ifetch_buffer_index].branch_type == BRANCH_CONDITIONAL && branch_prediction == 1)
                                {
                                    IFETCH_BUFFER.entry[ifetch_buffer_index].resolve_cond_branch_in_decode = 1;
                                }
                            }
                        }
                        else
				    ...
				    IFETCH_BUFFER.entry[ifetch_buffer_index].branch_mispredicted = 1;
                        ...
                        update_btb(IFETCH_BUFFER.entry[ifetch_buffer_index].ip,
...
void O3_CPU::spec_inst_prefetch()  ///Yash - Diamond function of the decoupled front end - recurive predictons sent to the FDIP, but wait, the thing 
{
    //return;
    if (skip_next_cycle)
    {
        skip_next_cycle = false;
        //		cout << "Skipping cycle1 " << current_core_cycle[cpu] << endl;
        return;
    }
    if (IBUF_occupancy < IFETCH_BUFFER.SIZE)
    {
        for (int i = 0; i < FETCH_WIDTH; i++)
        {
            IBUF_occupancy++;
            l1i_prefetcher_instruction_spec_operate(nextInstPC);

            BTB_reads++;
            BTB_outcome btb_result = btb_prediction(nextInstPC, 0);
            uint64_t predicted_branch_target = btb_result.target;
            uint8_t branch_type = btb_result.branch_type;
            uint8_t branch_prediction = predict_branch(nextInstPC);
            if (branch_type != BRANCH_CONDITIONAL)
            {
                branch_prediction = 1;
            }
            ...
            if (branch_prediction && btb_result.sequential_BTB_access == 1)
            {
                //                        	cout << "Need to skip next cycle1. Current " << current_core_cycle[cpu] << endl;
                skip_next_cycle = true;
            }

            if (branch_prediction && predicted_branch_target)
            {
                nextInstPC = predicted_branch_target;
                break;
            }
            else
            {
...
void O3_CPU::decode_and_dispatch()
{
    ...
    for (uint32_t i = 0; i < DECODE_BUFFER.SIZE; i++)
    {
            ...
            ((DECODE_BUFFER.entry[DECODE_BUFFER.head].event_cycle != 0) && (DECODE_BUFFER.entry[DECODE_BUFFER.head].event_cycle < current_core_cycle[cpu]) && (ROB.occupancy < ROB.SIZE)))
        {
            ...
            {
                if (ROB.entry[rob_index].branch_type == BRANCH_DIRECT_JUMP || ROB.entry[rob_index].branch_type == BRANCH_DIRECT_CALL || ROB.entry[rob_index].resolve_cond_branch_in_decode)
                {
...
int O3_CPU::prefetch_code_line(uint64_t pf_v_addr){///Yash O3/L1I interface
    if (pf_v_addr == 0)
    {
    ...
    L1I.pf_requested++;  ///Yash STATS IMP 
...
void O3_CPU::operate_cache()
{
    ...
    l1i_prefetcher_cycle_operate();///Called at the same time as the other caches ///Yash L1I Operate
}

void O3_CPU::update_rob()///Yash
{
...
void O3_CPU::complete_instr_fetch(PACKET_QUEUE *queue, uint8_t is_it_tlb)///Yash
{
