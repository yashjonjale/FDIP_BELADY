...
// CORE PROCESSOR
#define FETCH_WIDTH 6  ///Yash IMP
#define DECODE_WIDTH 6  ///Yash Superscalar?
#define EXEC_WIDTH 6
#define LQ_WIDTH 2
#define SQ_WIDTH 2
...
class O3_CPU {
  public:
    ...
    uint64_t instr_unique_id, completed_executions, ///Yash STATS IMP
    ...
    uint8_t  fetch_stall; ///Yash - maybe helpful
    uint64_t fetch_resume_cycle;///Yash - maybe helpful
    uint64_t nextInstPC; ///Yash - maybe helpful
    uint32_t IBUF_occupancy;  ///Yash - maybe helpful
    uint64_t num_branch, branch_mispredictions, branch_direction_mispredictions, branch_target_mispredictions1, branch_target_mispredictions2;
    uint64_t mispredict_on_btb_hit;  ///Yash - maybe helpful
    uint64_t total_rob_occupancy_at_branch_mispredict;
  uint64_t total_branch_types[8];
  uint64_t mispredicted_branch_types[8];///Yash - STATS IMP
  ...
  std::map<uint64_t, std::set<uint64_t>> branchesInCB;
  std::set<uint64_t> unique_branchPC;
    uint64_t offset_size[132];
    uint64_t BTB_4D_Hits, BTB_6D_Hits, BTB_8D_Hits, BTB_12D_Hits, BTB_18D_Hits, BTB_25D_Hits, BTB_46D_Hits, BTB_Ret_Hits;
    uint64_t BTB_4D_Misses, BTB_6D_Misses, BTB_8D_Misses, BTB_12D_Misses, BTB_18D_Misses, BTB_25D_Misses, BTB_46D_Misses, BTB_Ret_Misses;
  ...
  // trace cache for previously decoded instructions
	...
	fetch_resume_cycle = 0;
        ...
        branch_direction_mispredictions = 0;
        branch_target_mispredictions1 = 0;
        branch_target_mispredictions2 = 0;
        mispredict_on_btb_hit = 0;
	for(uint32_t i=0; i<8; i++)
	  {
	    total_branch_types[i] = 0;
	    mispredicted_branch_types[i] = 0;
	  }
	BTB_reads = BTB_writes = PageBTB_reads = PageBTB_writes = PageBTB_readsBeforeWrite = RegionBTB_reads = RegionBTB_writes = RegionBTB_readsBeforeWrite = 0;
	branchesInCB.clear();
	unique_branchPC.clear();
	for(uint32_t i=0; i<132; i++)
	  {
	    offset_size[i] = 0;
	  }

	BTB_4D_Hits = BTB_6D_Hits = BTB_8D_Hits = BTB_12D_Hits = BTB_18D_Hits = BTB_25D_Hits = BTB_46D_Hits = BTB_Ret_Hits = 0;
	BTB_4D_Misses = BTB_6D_Misses = BTB_8D_Misses = BTB_12D_Misses = BTB_18D_Misses = BTB_25D_Misses = BTB_46D_Misses = BTB_Ret_Misses = 0;
